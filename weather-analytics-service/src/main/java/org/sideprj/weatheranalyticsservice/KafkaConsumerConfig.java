...
public class KafkaConsumerConfig {
    ...
    private Map<String, Object> getBootstrapServersConfig(Class<?> classType) {
    ...
    public ConsumerFactory<String, WeatherEvent> weatherEventConsumerFactory() {
        return new DefaultKafkaConsumerFactory<>(
                getBootstrapServersConfig(WeatherEvent.class)
        );
    }
    ...
    public ConcurrentKafkaListenerContainerFactory<String, WeatherEvent> weatherEventContainerFactory(
            ...
            @Value("${kafka.weather.topic.hot-weather}") String topic
    ) {
        ...
        factory.setRecordFilterStrategy(eventRecord -> {
            log.info("Received message: {}", eventRecord.value());
            if (eventRecord.value().getTemp() > HOT_TEMPERATURE) {
                kafkaTemplate.send(topic, WeatherEventMapper.toHotWeatherAlert(eventRecord.value()));
                return false;
            }
            return false;
        });
    ...
    @Bean
    public ConsumerFactory<String, HotWeatherAlertEvent> hotWeatherAlertConsumerFactory() {
        return new DefaultKafkaConsumerFactory<>(
                getBootstrapServersConfig(HotWeatherAlertEvent.class)
        );
    }

    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, HotWeatherAlertEvent> hotWeatherAlertContainerFactory() {
        ConcurrentKafkaListenerContainerFactory<String, HotWeatherAlertEvent> factory
                = new ConcurrentKafkaListenerContainerFactory<>();
        factory.setConsumerFactory(hotWeatherAlertConsumerFactory());
        return factory;
    }
}
